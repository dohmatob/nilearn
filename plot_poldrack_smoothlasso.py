"""
S-LASSO regression example on Poldrack's Mixed Gambles dataset.
...
tvl1cv = SmoothLassoRegressorCV(n_alphas=10, tol=1e-4, max_iter=1000,
                                l1_ratio=.75, verbose=1, memory=memory,
                                mask=mask, n_jobs=n_jobs).fit(X, y)

# Retrieve the S-LASSO discriminating weights
